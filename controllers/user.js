...
export const registration=asyncError(
    async(req,res,next)=>{
        const {
            name,
            email,password,
...
export const updateProfile=asyncError(
    async(req,res,next)=>{
        ...
        res.status(200).json({
            success:true,
            menubar:"Profile update Successfully",
        })
    }
)
...
export const forgetpassword=asyncError(
    async(req,res,next)=>{
        ...
        const randomNumber=Math.random()*(999999-10000)+100000;
        const otp=Math.floor(randomNumber);
        const otp_expire=15*60*1000;
        user.otp=otp;
        user.otp_expire=new Date(Date.now()+otp_expire);
        await user.save();
        const message=`Your OTP For Reseting Password is ${otp}. Please Ignore If You Haven't requested this.`;
        try {
...
export const resetpassword=asyncError(
    ...
    async(req,res,next)=>{
        ...
        if(!user) return next(new ErrorHandler("Please Enter New password",400));
